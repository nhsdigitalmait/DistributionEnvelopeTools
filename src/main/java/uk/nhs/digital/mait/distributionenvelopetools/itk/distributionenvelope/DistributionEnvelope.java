...
public class DistributionEnvelope
     ...
     * prefix "itk" for those nodes in the ITK namespace. If a sender has a need
     * for some other prefix to be used, it should be set here.
     *
    ...
    /**
     * Called by the DistributionEnvelopeHelper to set the recipients list after
     * it has been parsed out of the received DistributionEnvelope XML.
     *
     ...
     * after it has been parsed out of the received DistributionEnvelope XML.
     *
     * @param id Audit Identity Instances
     */
    ...
    /**
     * Called by the DistributionEnvelopeHelper to set the sender address after
     * it has been parsed out of the received DistributionEnvelope XML.
     *
     * @param a ender Address
     */
    protected void setSender(Address a) {
        sender = a;
    }

    /**
     * Called by the DistributionEnvelopeHelper to set the text of the received
     * DistributionEnvelope XML after other data have been parsed out.
     *
     * @param d DistributionEnvelope XML as a string
     */
    void setDistributionEnvelope(String d) {
        envelope = d;
    }

    /**
     * Called by the DistributionEnvelopeHelper to set the tracking id of a
     * received DistributionEnvelope.
     *
     * @param t
     */
    protected void setTrackingId(String t) {
        trackingId = t;
    }

    /**
     * Called by the DistributionEnvelopeHelper to set the service of a received
     * DistributionEnvelope, or by a builder to set the service attribute.
     *
     * @param s
     */
    ...
    /**
     * Adds a handling specification given the type and value. This does not
     * validate that the given type is defined in the ITK specifications. Sets
     * the "interactionId" if the supplied type is the identifier for an ITK
     * interactionId.
     *
     * @param s
     * @param v
    ...
    public void addHandlingSpecification(String s, String v) {
        if (handlingSpecification == null) {
            handlingSpecification = new HashMap<>();
        }
    ...
    /**
     * @return @returns The XML text of the DistributionEnvelope.
     */
    ...
    /**
     * @return @returns the interactionId (note that this MAY be null, and it is
     * not an error for a DistributionEnvelope to have no interactionId, so
     * applications which require interactionId are responsible for checking for
     * null returns from this method.
     */
    ...
    /**
     * @return @returns Array of recipient Address objects, may be empty or
     * null.
     */
    ...
    /**
     * @return @returns Array of author audit Identity objects. May be empty or
     * null.
     */
    ...
    /**
     * @return @returns Sender Address object. May be null.
     */
    ...
    /**
     * Used by senders to add recipient addresses. Any address type may be
     * entered, described by the appropriate OID. Where the OID is null, the
     * default "ITK address" is supplied.
     *
    ...
    /**
     * Used by senders to add sender identities. Any identity type may be
     * entered, described by the appropriate OID. Where the OID is null, the
     * default "ITK identity" is supplied.
     *
    ...
    /**
     * (This should probably be called setSender()) Used by the sender to set
     * the sender address. Any address type may be entered, described by the
     * appropriate OID. Where the OID is null, the default "ITK address" is
     * supplied.
     *
     * @param oid OID for the address type, or null
     * @param id Address
    ...
    /**
     * Called by the sender to set the ITK interaction id. This sets the
     * appropriate handlingSpecification. The supplied value is NOT validated
     * against any list of known, defined ITK interaction ids.
     *
    ...
    /**
     * When the DistributionEnvelopeHelper is used to construct a
     * DistributionEnvelope from a received message, it does not parse the
     * payloads themselves. So the DistributionEnvelope instance contains no
     * Payload objects. If these are required, the parsePayloads() method is
     * called to parse out the payloads from the received XML.
     *
     * @throws java.lang.Exception
     */
...
//    }
    /**
     * Convenience method called by a sender to explicitly set the "ack
     * requested" handling specification.
     *
     * @param b
     */
     ...
     * @return Value, or null if that handling specification is not set.
     */
